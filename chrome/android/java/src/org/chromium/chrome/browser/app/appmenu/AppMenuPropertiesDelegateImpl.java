...
public class AppMenuPropertiesDelegateImpl implements AppMenuPropertiesDelegate {
    ...
    public void prepareMenu(Menu menu, AppMenuHandler handler) {
        ...
        disableMenuItem(menu, R.id.add_bookmark_menu_id);
        disableMenuItem(menu, R.id.all_bookmarks_menu_id);
        disableMenuItem(menu, R.id.bookmark_this_page_id);
        disableMenuItem(menu, R.id.offline_page_id);
        disableMenuItem(menu, R.id.downloads_menu_id);
        disableMenuItem(menu, R.id.edit_bookmark_menu_id);
        disableMenuItem(menu, R.id.find_in_page_id);
        disableMenuItem(menu, R.id.help_id);
        disableMenuItem(menu, R.id.new_incognito_tab_menu_id);
        disableMenuItem(menu, R.id.open_history_menu_id);
        disableMenuItem(menu, R.id.request_desktop_site_row_menu_id);
        disableMenuItem(menu, R.id.share_row_menu_id);
        disableMenuItem(menu, R.id.info_menu_id);
        disableMenuItem(menu, R.id.downloads_menu_id);
        disableDividers(menu);
    }

    private void disableMenuItem(Menu menu, int ids) {
        menu.findItem(ids).setVisible(false).setEnabled(false).setIcon(null);
    }

    private void disableDividers(Menu menu) {
        for (int i = 0; i < menu.size(); ++i) {
            MenuItem item = menu.getItem(i);
